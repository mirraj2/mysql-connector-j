...
public abstract class AbstractResultsetRow implements ResultsetRow {
  ...
  private <T> T decodeAndCreateReturnValue(int columnIndex, byte[] bytes, int offset, int length, ValueFactory<T> vf) {
    ...
    case MysqlType.FIELD_TYPE_TINY:
      return f.isUnsigned() ? this.valueDecoder.decodeUInt1(bytes, offset, length, vf)
    ...
    case MysqlType.FIELD_TYPE_SHORT:
      return f.isUnsigned() ? this.valueDecoder.decodeUInt2(bytes, offset, length, vf)
          : this.valueDecoder.decodeInt2(bytes, offset, length, vf);

    case MysqlType.FIELD_TYPE_LONG:
      return f.isUnsigned() ? this.valueDecoder.decodeUInt4(bytes, offset, length, vf)
    ...
    case MysqlType.FIELD_TYPE_LONGLONG:
      return f.isUnsigned() ? this.valueDecoder.decodeUInt8(bytes, offset, length, vf)
  ...
  /**
   * Get a value from a byte array. The byte array is interpreted by the {@link com.mysql.cj.protocol.ValueDecoder}
   * which uses the value factory create the return value.
   *
