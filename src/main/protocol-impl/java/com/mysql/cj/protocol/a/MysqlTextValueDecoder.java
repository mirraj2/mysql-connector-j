...
/**
 * Implementation of {@link com.mysql.cj.protocol.ValueDecoder} for the MySQL text protocol. All values will be received
 * as <i>LengthEncodedString</i> values.
 * <p>
...
public class MysqlTextValueDecoder implements ValueDecoder {
  ...
  /**
   * String length of String timestamp with nanos. This does not come from MySQL server but we support it via string
   * conversion.
   */
  ...
  public static int getInt(byte[] buf, int offset, int endpos) throws NumberFormatException {
    return (int) getLong(buf, offset, endpos);
  }

  public static final int base = 10;
  public static final long cutoff = Long.MAX_VALUE / base;
  ...
  public static long getLong(byte[] buf, int offset, int endpos) throws NumberFormatException {
    ...
    if (s == endpos) {
      throw new NumberFormatException(
          Messages.getString("StringUtils.badIntFormat",
    ...
    for (; s < endpos; s++) {
      ...
      // throw new NumberOutOfRange(Messages.getString("StringUtils.badIntFormat",
      // new Object[] { StringUtils.toString(buf, offset, endpos - offset) }));
      // }
    }
  ...
  public static boolean isDate(String s) {
    return s.length() == DATE_BUF_LEN && s.charAt(4) == '-' && s.charAt(7) == '-'; // TODO also check proper date parts
                                                                                   // ranges
  }
  ...
  public static InternalTime getTime(byte[] bytes, int offset, int length, int scale) {
    ...
    if (segmentLen == 0 || bytes[offset + pos + segmentLen] != ':') {
      throw new DataReadException(
          Messages.getString("ResultSet.InvalidFormatForType",
    ...
    // parse seconds field
    for (segmentLen = 0; offset + pos + segmentLen < offset + length
        && Character.isDigit((char) bytes[offset + pos + segmentLen]); segmentLen++) {
    ...
    if (length > pos) {
      ...
      // scale out nanos appropriately. mysql supports up to 6 digits of fractional seconds, each additional digit
      // increasing the range by a factor of
  ...
  public static InternalTimestamp getTimestamp(byte[] bytes, int offset, int length, int scale) {
    ...
    if (bytes[offset + 4] != (byte) '-' || bytes[offset + 7] != (byte) '-' || bytes[offset + 10] != (byte) ' '
