...
public class ResultSetImpl extends NativeResultset implements ResultSetInternalMethods, WarningListener {
  ...
  /**
   * First character of the query that created this result set...Used to determine whether or not to parse server info
  ...
  public ResultSetImpl(OkPacket ok, JdbcConnection conn, StatementImpl creatorStmt) {
    ...
    if (this.connection != null) {
      ...
      this.padCharsWithSpace = this.connection.getPropertySet().getBooleanProperty(PropertyKey.padCharsWithSpace)
          .getValue();
    }
  }
  ...
  public ResultSetImpl(ResultsetRows tuples, JdbcConnection conn, StatementImpl creatorStmt) throws SQLException {
    ...
    this.defaultUtilCalendarValueFactory = new UtilCalendarValueFactory(pset,
        this.session.getServerSession().getDefaultTimeZone(),
        this.session.getServerSession().getSessionTimeZone());
    ...
    this.defaultOffsetTimeValueFactory = new OffsetTimeValueFactory(pset,
        this.session.getProtocol().getServerSession().getDefaultTimeZone());
    this.defaultOffsetDateTimeValueFactory = new OffsetDateTimeValueFactory(pset,
        this.session.getProtocol().getServerSession().getDefaultTimeZone(),
        this.session.getProtocol().getServerSession().getSessionTimeZone());
    this.defaultZonedDateTimeValueFactory = new ZonedDateTimeValueFactory(pset,
        this.session.getProtocol().getServerSession().getDefaultTimeZone(),
        this.session.getProtocol().getServerSession().getSessionTimeZone());
  ...
  protected void checkRowPos() throws SQLException {
    ...
    if (!this.onValidRow) {
      throw SQLError.createSQLException(this.invalidRowReason, MysqlErrorNumbers.SQL_STATE_GENERAL_ERROR,
  ...
  public Reader getCharacterStream(int columnIndex) throws SQLException {
    ...
    try {
      ...
      SQLException sqlEx = SQLError.createSQLException("Cannot read value with encoding: " + f.getEncoding(),
  ...
  public Date getDate(int columnIndex) throws SQLException {
    ...
    return this.thisRow.getValue(columnIndex - 1,
        new SqlDateValueFactory(this.session.getPropertySet(), null,
            this.session.getServerSession().getDefaultTimeZone(), this));
  }
  ...
  public BigInteger getBigInteger(int columnIndex) throws SQLException {
    ...
    try {
      ...
      throw SQLError.createSQLException(
          Messages.getString("ResultSet.Bad_format_for_BigInteger",
  ...
  private String getStringForClob(int columnIndex) throws SQLException {
    ...
    String forcedEncoding = this.connection.getPropertySet().getStringProperty(PropertyKey.clobCharacterEncoding)
        .getStringValue();
  ...
  public Timestamp getTimestamp(int columnIndex, Calendar cal) throws SQLException {
    checkRowPos();
    checkColumnBounds(columnIndex);
    ValueFactory<Timestamp> vf = new SqlTimestampValueFactory(this.session.getPropertySet(), cal,
        this.session.getServerSession().getDefaultTimeZone(),
        this.session.getServerSession().getSessionTimeZone());
    return this.thisRow.getValue(columnIndex - 1, vf);
  }
  ...
  @Override
  public Object getObjectStoredProc(String colName, java.util.Map<Object, Object> map, int desiredSqlType)
      throws SQLException {
  ...
  public java.sql.Statement getStatement() throws SQLException {
    try {
      ...
      throw SQLError.createSQLException("Operation not allowed on closed ResultSet.",
          MysqlErrorNumbers.SQL_STATE_GENERAL_ERROR,
          getExceptionInterceptor());
    }
  }
  ...
  /**
   * Checks whether this ResultSet is scrollable even if its type is ResultSet.TYPE_FORWARD_ONLY. Required for backwards
   * compatibility.
   *
   * @return <code>true</code> if this result set type is ResultSet.TYPE_FORWARD_ONLY and the connection property
   *         'scrollTolerantForwardOnly' has not been set to <code>true</code>.
   */
  ...
  /**
   * The <i>prev</i> method is not part of JDBC, but because of the architecture of this driver it is possible to move
   * both forward and backward within the result set.
   *
   * <p>
   * If an input stream from the previous row is open, it is implicitly closed. The ResultSet's warning chain is cleared
    ...
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      if (isStrictlyForwardOnly() && direction != FETCH_FORWARD) {
        ...
        throw ExceptionFactory.createException(
            Messages.getString("ResultSet.Unacceptable_value_for_fetch_direction", new Object[] { constName }));
      }
