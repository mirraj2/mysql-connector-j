...
public class UpdatableResultSet extends ResultSetImpl {
  ...
  public void checkUpdatability() throws SQLException {
    try {
      if (this.getMetadata() == null) {
        // we've been created to be populated with cached metadata, and we don't have the metadata yet, we'll be called
        // again by
      ...
      // We can only do this if we know that there is a currently selected database, or if we're talking to a > 4.1
      // version of MySQL server (as it returns
      // database names in field info)
      if (this.db == null || this.db.length() == 0) {
        this.db = fields[0].getDatabaseName();

        if (this.db == null || this.db.length() == 0) {
          throw SQLError.createSQLException(Messages.getString("UpdatableResultSet.43"),
      ...
      if (this.getSession().getPropertySet().getBooleanProperty(PropertyKey.strictUpdates).getValue()) {
        ...
        try {
          rs = this.session.getPropertySet().<DatabaseTerm>getEnumProperty(PropertyKey.databaseTerm)
  ...
  private void setParamValue(ClientPreparedStatement ps, int psIdx, Row row, int rsIdx, Field field)
    ...
    case BIT:
      ...
      break;
    /*
     * default, but also explicitly for following types: case Types.BINARY: case Types.BLOB:
     */
    default:
  ...
  private void extractDefaultValues() throws SQLException {
    ...
    for (Map.Entry<String, Map<String, Map<String, Integer>>> dbEntry : this.databasesUsedToTablesUsed.entrySet()) {
      for (Map.Entry<String, Map<String, Integer>> tableEntry : dbEntry.getValue().entrySet()) {
        ...
        try {
          columnsResultSet = this.session.getPropertySet().<DatabaseTerm>getEnumProperty(PropertyKey.databaseTerm)
              .getValue() == DatabaseTerm.SCHEMA
  ...
  /**
   * Figure out whether or not this ResultSet is updatable, and if so, generate the PreparedStatements to support
   * updates.
   *
   * @throws SQLException if an error occurs
  ...
  protected void generateStatements() throws SQLException {
    ...
    for (int i = 0; i < fields.length; i++) {
      ...
      String columnName = this.hasLongColumnInfo && originalColumnName != null && originalColumnName.length() > 0
          ? originalColumnName
          : fields[i].getName();
      ...
      String tableName = this.hasLongColumnInfo && originalTableName != null && originalTableName.length() > 0
          ? originalTableName
    ...
    this.updateSQL = "UPDATE " + this.qualifiedAndQuotedTableName + " " + fieldValues.toString() + " WHERE "
        + keyValues.toString();
    this.insertSQL = "INSERT INTO " + this.qualifiedAndQuotedTableName + " (" + columnNames.toString() + ") VALUES ("
        + insertPlaceHolders.toString() + ")";
    this.refreshSQL = "SELECT " + columnNames.toString() + " FROM " + this.qualifiedAndQuotedTableName + " WHERE "
        + keyValues.toString();
  ...
  private Map<String, Integer> getColumnsToIndexMapForTableAndDB(String databaseName, String tableName) {
    ...
    if (tablesUsedToColumnsMap == null) {
      tablesUsedToColumnsMap = this.session.getServerSession().isLowerCaseTableNames()
  ...
  public void insertRow() throws SQLException {
    synchronized (checkClosed().getConnectionMutex()) {
      ...
      for (int i = 0; i < fields.length; i++) {
        ...
        // WARN: This non-variant only holds if MySQL never allows more than one auto-increment key (which is the way it
        // is _today_)
      ...
      // inserter is always a client-side prepared statement, so it's safe to use it with ByteArrayRow for server-side
  ...
  private void refreshRow(ClientPreparedStatement updateInsertStmt, Row rowToRefresh) throws SQLException {
    if (this.refresher == null) {
      ...
      // We're going to copy bytes from refresher results to rowToRefresh, thus we need them to have the same protocol
      // encoding
    ...
    try {
      ...
      if (rs.next()) {
        ...
        throw SQLError.createSQLException(Messages.getString("UpdatableResultSet.12"),
  ...
  protected void syncUpdate() throws SQLException {
    ...
    for (int i = 0; i < numFields; i++) {
      if (this.thisRow.getBytes(i) != null) {
        ...
        case TIME:
        case TIMESTAMP:
          // TODO this is a temporary workaround until Bug#71143 "Calling ResultSet.updateRow should not set all field
          // values in UPDATE" is fixed.
          // We handle these types separately to avoid fractional seconds truncation (when sendFractionalSeconds=true)
          // that happens for fields we don't touch with ResultSet.updateNN(). For those fields we should pass the value
          // as is or,
  ...
  /**
   * Internal setObject implementation. Although targetType is not part of default ResultSet methods signatures, it is
   * used for type conversions from JDBC42UpdatableResultSet new JDBC 4.2 updateObject() methods.
   *
